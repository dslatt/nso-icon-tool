...
struct CategoryPart {
  std::string name;
  std::string icon;
  CategoryPart(std::string name, std::string icon)
      : name(name)
      , icon(icon)
  {
  }
...
public:
  ...
  virtual void prepareForReuse() override { image->clear(); }

  virtual void cacheForReuse() override { image->clear(); }
...
public:
  DataSource(std::vector<CategoryPart> parts, std::function<void(std::string)> onSelected,
...
public:
  IconPartSelect(const std::vector<CategoryPart>& files, std::string subcategory, const ImageState& state,
