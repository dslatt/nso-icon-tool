...
public:
  DataSourceSkeleton(unsigned int n)
      : num(n)
  {
  }
...
RecyclingGrid::RecyclingGrid()
{
  ...
  this->registerPercentageXMLAttribute(
      "paddingRight", [this](float percentage) { this->setPaddingRightPercentage(percentage); });

  this->registerPercentageXMLAttribute(
      "paddingLeft", [this](float percentage) { this->setPaddingLeftPercentage(percentage); });
...
void RecyclingGrid::reloadData()
{
  ...
  if (!isFlowMode || spanCount != 1) {
    // 设置了固定的高度
    contentBox->setHeight((estimatedRowHeight + estimatedRowSpace) * (float)getRowCount() + paddingTop + paddingBottom);
    // 添加当前焦点 cell 所在行的第一项到屏幕，其余项通过 selectRowAt 内的 itemsRecyclingLoop 自动添加
    // 这里添加首项是因为添加首项时会变更 renderedFrame 的 height 值，包括 itemsRecyclingLoop
    // 内的计算也都是以首项为基准进行的 原则上这里的 addCellAt 任意添加一项即可（比如添加第零项），但最好能添加到
    // cellFocusIndex 附近，这有助于提升首屏性能
    ...
    // 添加 lineHeadIndex 项到需要渲染的列表项中，因为 lineHeadIndex 为一行的首项，在执行 addCellAt 之后会更新
    // renderedFrame 数据 renderedFrame 的 height 调整为第 lineHeadIndex 项的高度
    ...
    // 流式布局无法准确确定焦点cell的位置，因此暂时只添加第一项，在 itemsRecyclingLoop 中会逐渐添加到
    // cellFocusIndex，再按需删除 当 cellFocusIndex 过于大时，会导致添加的元素过多，因此需要考虑优化
...
void RecyclingGrid::itemsRecyclingLoop()
{
  ...
  while (true) {
    ...
    if (!maxCell
        || (maxCell->getDetachedPosition().y - getHeightByCellIndex(visibleMax, visibleMax - preFetchLine * spanCount)
  ...
  while (visibleMax + 1 < dataSource->getItemCount()) {
    ...
    if ((visibleMax + 1) % spanCount == 0)
      ...
      if (renderedFrame.getMaxY() - getHeightByCellIndex(visibleMax + 1, visibleMax + 1 - preFetchLine * spanCount)
...
brls::View* RecyclingGrid::getNextCellFocus(brls::FocusDirection direction, brls::View* currentView)
{
  ...
  if ((this->contentBox->getAxis() == brls::Axis::ROW && direction != brls::FocusDirection::LEFT
    ...
    while (
        !row_currentFocus && row_currentFocusIndex >= 0 && row_currentFocusIndex < this->dataSource->getItemCount()) {
  ...
  // Return nullptr immediately if focus direction mismatches the box axis (clang-format refuses to split it in multiple
  // lines...)
  if ((this->contentBox->getAxis() == brls::Axis::ROW && direction != brls::FocusDirection::LEFT
          && direction != brls::FocusDirection::RIGHT)
