...
RecyclerCell::RecyclerCell() { this->inflateFromXMLRes("xml/cells/icon_part_cell.xml"); }
...
void DataSource::onItemSelected(RecyclingGrid* recycler, size_t index)
{
  ...
  if (parts[index].name == "none" && parent) {
    ...
    if (files.empty()) {
      ...
      recycler->present(new grid::IconPartSelectGrid(
          files, "app/main/available_images"_i18n, state,
          [this](std::string icon) {
            onSelected(icon);
            if (parent) {
              parent->dismiss();
            }
    ...
    auto files = std::ranges::subrange(
                     fs::directory_iterator(fs::path(paths::IconCachePath) / parts[index].name / subcategory),
    ...
    recycler->present(new grid::IconPartSelectGrid(
        files, convertName(parts[index].name), state,
        [this](std::string icon) {
          onSelected(icon);
          if (parent) {
            parent->dismiss();
...
size_t DataSource::getItemCount() { return parts.size(); }

void DataSource::clearData() { parts.clear(); }

DataSource::DataSource(std::vector<CategoryPart> parts, std::function<void(std::string)> onSelected,
    std::function<void(std::string, ImageState& state)> onFocused, brls::View* parent, std::string subcategory,
    ImageState& state)
    : parent(parent)
...
IconPartSelect::IconPartSelect(const std::vector<CategoryPart>& files, std::string subcategory, const ImageState& state,
